...
class Runner {
    ...
    shouldExecuteSpec (spec) {
        // add support for 4* ...

        console.log('shouldExecuteSpec: ', spec)

        /* console.log('execution filters: ', this.config.executionFilters)

        if (this.config.executionFilters.specs) {
            console.log('should execute: ', spec)
            return true
        } else {
            console.log('should not execute: ', spec)
            return false
        } */
    }

    shouldExecuteTestcase (testcase) {
    ...
    async run (m) {
        ...
        global.getUid = this.getUid

        // does not work - inject in before helper??
        global._workflo = {}
        process._workflo.shouldExecuteSpec = this.shouldExecuteSpec
        process._workflo.shouldExecuteTestcase = this.shouldExecuteTestcase
