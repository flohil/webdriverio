...
class Launcher {
    ...
    getPatchedStack (previousStack, cid) {
        var self = this
        var combinedStack = previousStack + '\n' + this.stackStore[cid]
    ...
    messageHandler (cid, m) {
        ...
        if (this.finishedTests) {
            ...
            if (m.event === "runner:currentStack") {
                if (!this.stackStoreLocks[cid]) {
                    this.stackStore[cid] = m.stack
                }
            } else if (m.event === "runner:aftercommand" && m.err) {
                this.stackStoreLocks[cid] = true
            }

            if (m.event === 'test:start') {
                m.testcase = m.specs[0]
                this.stackStoreLocks[cid] = false
                delete this.stackStore[cid]
            }
                        ...
                        if (err.specObj) {
                            for (const key of Object.keys(err.specObj)) {
                                ...
                                for (const criteria of err.specObj[key]) {
                                    ...
                                    if (!err.matcherName) {
                                        err.stack = this.getPatchedStack(err.stack, m.cid)
                                    }
            ...
            if (m.event === 'step:broken') {
                ...
                // the step broke
                m.assertion.stack = this.getPatchedStack(m.assertion.stack, m.cid)
