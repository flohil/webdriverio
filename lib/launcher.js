        ...
        if (this.finishedTests) {
            ...
            case 'step:end':
                ...
                if (m.validate) {
                    ...
                    this.verificationStates[this.getCidGroup(m.cid)].storyId = m.validate.storyId
                    this.verificationStates[this.getCidGroup(m.cid)].criteriaId = m.validate.criteriaId

                    const status = this.evaluateVerifications(m.validate.storyId, m.validate.criteriaId, m)
                    m.event = `step:${status}`
                }
                break
            case 'test:pass':
