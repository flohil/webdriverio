...
class Launcher {
    ...
    messageHandler (cid, m) {
        ...
        if (this.finishedTests) {
            console.log('finished Tests', m.event)
            ...
            case 'test:pass':
                ...
                console.log('bla: ', story)

                console.dir(this.verificationResults, {depth: null})
                console.log('message: ', m)

                if (story.failures[criteriaId].length > 0) {
                    ...
                    err.message = story.failures[criteriaId].map((err) => err.message).reduce((prev, cur) => prev + '\n' + cur)
                    err.stack = story.failures[criteriaId].map((err) => err.stack).reduce((prev, cur) => prev + '\n\n' + cur)
                    ...
                    console.log('failed')
                } else if (story.failures[criteriaId].length === 0 && story.successes[criteriaId].length > 0) {
