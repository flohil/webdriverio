...
class Launcher {
        ...
        while (this.getNumberOfRunningInstances() < config.maxInstances) {
            let schedulableCaps = this.schedule
                ...
                .filter(() => {
                    ...
                    if (this.argv.bailErrors && config.workfloBail && this.argv.bailErrors <= config.workfloBail) {
                        this.argv.bailRunner = true
                    }
    ...
    startValidatorInstance (specs, caps, cid, server) {
        ...
        if (config.execArgv) {
            config.execArgv = config.execArgv.filter( param => param.indexOf('--inspect') === -1)
        }
    ...
    messageHandler (cid, m) {
        ...
        if (this.finishedTests) {
            ...
            if (m.event === 'test:fail') {
                ...
                if (!this.argv['bailErrors']) {
                    this.argv['bailErrors'] = 1
                } else {
                    this.argv['bailErrors'] += 1
                }
            }
            ...
            if (m.event === 'step:failed' || m.event === 'step:broken' || m.event === 'retry:validateFailure') {
            ...
            if (m.event === 'step:failed' || m.event === 'step:broken') {
                ...
                if (m.assertion.screenshotId) {
                    this.argv.screenshotId = m.assertion.screenshotId
                }
            }
            ...
            if (m.event === 'retry:resetErrors') {
                this.stepErrors[this.getCidGroup(m.cid)].broken = []
                this.stepErrors[this.getCidGroup(m.cid)].failed = []
            }

            if (m.event === 'retry:failed' || m.event === 'retry:broken') {
                if (m.event === 'retry:failed') {
                    m.status = 'failed'
                }
                if (m.event === 'retry:broken') {
                    m.status = 'broken'
