...
class Launcher {
    ...
    async run () {
        ...
        // this.reporters.handleEvent('verificationResults', this.getVerificationResults())
    ...
    getVerificationResults () {
        return this.verificationResults['0-0']
    }
    ...
    messageHandler (cid, m) {
        ...
        if (this.finishedTests) {
            ...
            case 'step:end':
                ...
                if (m.verify) {
                    if (!this.verificationStates[m.cid]) {
                        this.verificationStates[m.cid] = {}
                    }
                    this.verificationStates[m.cid].storyId = m.verify.storyId
                    this.verificationStates[m.cid].criteriaId = m.verify.criteriaId
                ...
                break
            case 'test:pass':
                const storyId = this.verificationStates[m.cid].storyId
                const criteriaId = this.verificationStates[m.cid].criteriaId
            ...
            if (m.event === 'test:fail') {
                if (m.errs) {
                    ...
                    if (!(m.cid in this.verificationResults)) {
                        this.verificationResults[m.cid] = {}
                    }

                    for (const err of m.errs) {
                        if (err.specObj) {
                            for (const key of Object.keys(err.specObj)) {
                                if (!(key in this.verificationResults[m.cid])) {
                                    this.verificationResults[m.cid][key] = {
                                        successes: {},
                                        failures: {}
                                    }
                                }
                                for (const criteria of err.specObj[key]) {
                                    if (!(criteria in this.verificationResults[m.cid][key])) {
                                        this.verificationResults[m.cid][key].failures[criteria] = []
                                    }
                                    this.verificationResults[m.cid][key].failures[criteria].push({
            ...
            if (m.event === 'step:start') {
                if (!this.stepStates[m.cid]) {
                    this.stepStates[m.cid] = {
                        stepLevel: 0
                    }
                }

                if (this.stepStates[m.cid].stepLevel === 0) {
                    this.stepStates[m.cid].levelStatus = []
                }

                this.stepStates[m.cid].stepLevel++

                if (this.stepStates[m.cid].levelStatus.length < this.stepStates[m.cid].stepLevel) {
                    this.stepStates[m.cid].levelStatus.push('passed')
                }
            }

            if (m.event === 'step:end') {
                const levelStatus = this.stepStates[m.cid].levelStatus

                if (levelStatus.length > this.stepStates[m.cid].stepLevel) {
                ...
                if (this.verificationStates[m.cid] && this.verificationStates[m.cid].assertionFailures) {
                    m.assertionFailures = this.verificationStates[m.cid].assertionFailures
                    m.status = 'failed'
                    levelStatus[levelStatus.length - 1] = 'failed'

                    this.verificationStates[m.cid].assertionFailures = undefined
                }
                ...
                for (const key of Object.keys(m.assertion.specObj)) {
                    ...
                    if (m.event === 'verify:success') {
                        for (const criteria of m.assertion.specObj[key]) {
                            if (!(criteria in this.verificationResults[m.cid][key].successes)) {
                                this.verificationResults[m.cid][key].successes[criteria] = []
                            }
                            this.verificationResults[m.cid][key].successes[criteria].push({
                        ...
                        for (const criteria of m.assertion.specObj[key]) {
                            if (!(criteria in this.verificationResults[m.cid][key].failures)) {
                                this.verificationResults[m.cid][key].failures[criteria] = []
                            }
                            if (!this.verificationStates[m.cid]) {
                                this.verificationStates[m.cid] = {}
                            }
                            if (!this.verificationStates[m.cid].assertionFailures) {
                                this.verificationStates[m.cid].assertionFailures = []
                            }

                            this.verificationStates[m.cid].assertionFailures.push(m.assertion)
                        }
                    }
                }
            } else {
    ...
    evaluateVerifications (storyId, criteriaId, m) {
        if (this.verificationResults && this.verificationResults['0-0'] && this.verificationResults['0-0'][storyId]) {
            const story = this.verificationResults['0-0'][storyId]
