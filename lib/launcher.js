...
class Launcher {
    ...
    mergeManualTestcaseResults () {
        ...
        for (const filename of config.manualTestcases) {
            const manualTestcase = require(filename)

            for (const story in manualTestcase.results) {
                if (this.verificationResults)
                for (const criteria in manualTestcase.results[story]) {

                    manualTestcase.results[story][criteria]
                }
            }
        }    
    }
                ...
                for (const key of Object.keys(m.assertion.specObj)) {
                    ...
                    if (m.event === 'verify:success') {
                        for (const criteria of m.assertion.specObj[key]) {
                            if (!(criteria in this.verificationResults[this.getCidGroup(m.cid)][key].successes)) {
                                this.verificationResults[this.getCidGroup(m.cid)][key].successes[criteria] = []
                            }
                            this.verificationResults[this.getCidGroup(m.cid)][key].successes[criteria].push({
                        ...
                        for (const criteria of m.assertion.specObj[key]) {
                            if (!(criteria in this.verificationResults[this.getCidGroup(m.cid)][key].failures)) {
                                this.verificationResults[this.getCidGroup(m.cid)][key].failures[criteria] = []
                            }
                            if (!this.verificationStates[this.getCidGroup(m.cid)]) {
                                this.verificationStates[this.getCidGroup(m.cid)] = {}
