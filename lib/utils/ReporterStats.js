...
class RunnableStats {
    constructor (type) {
        this.type = type
        this.start = new Date()
        this.testcaseStart = new Date()
        this._duration = 0
        this._specDuration = 0
        this._testcaseDuration = 0
    }
    ...
    getTestcaseDuration () {
        return new Date() - this.testcaseStart
    }

    getSpecsDuration () {
        return new Date() - this.specsStart
    }
}
...
class ReporterStats extends RunnableStats {
    ...
    reset (specs) {
        if (specs) {
            this.testcaseCounts = this.counts
            this.testcaseFailures = this.failures
            this.specsStart = new Date()
        } else {
            this.testcaseCounts = {
                suites: 0,
                tests: 0,
    ...
    getTestcaseFailures () {
        return this.testcaseFailures.map((test) => {
            test.runningBrowser = ''
    ...
    testUnverified (runner) {
        const testStats = this.getTestStats(runner)
        testStats.state = 'unverified'
        testStats.error = runner.err
        this.counts.unverifieds++

        runner.unverified = true
