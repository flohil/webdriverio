...
class ConfigParser {
    ...
    /**
     * extract workflo args from argv
     * @param {Object} object desired object to merge into the config object
     */
    extractExecutionFilterArgs (object = {}) {
        const executionFilters = {
            specs: (object.specs) ? JSON.parse(object.specs) : undefined,
            testcases: (object.testcases) ? JSON.parse(object.testcases) : undefined,
            listFiles: (object.listFiles) ? JSON.parse(object.listFiles) : undefined,
            specFiles: (object.specFiles) ? JSON.parse(object.specFiles) : undefined,
            testcaseFiles: (object.testcaseFiles) ? JSON.parse(object.testcaseFiles) : undefined
        }

        this._config = merge(this._config, {executionFilters: executionFilters})

        if (executionFilters.specFiles) {
            object.specs = executionFilters.specFiles
        }
        if (executionFilters.testcaseFiles) {
            object.testcases = executionFilters.testcaseFiles
        }

        delete object.specFiles
        delete object.testcaseFiles
        delete object.listFiles
     ...
     */
    merge (object = {}) {
        object = this.extractExecutionFilterArgs(object)
         ...
         */
        if (typeof object.specs !== 'undefined') {
            const specs = []

            for (let spec of object.specs) {
                const filename = path.resolve(this._config.testDir, 'src', 'specs', `${spec}.spec.ts`)
        ...
        if (typeof object.testcases !== 'undefined') {
            const testcases = []

            for (let testcase of object.testcases) {
                const filename = path.resolve(this._config.testDir, 'src', 'testcases', `${testcase}.tc.ts`)
                if (fs.existsSync(filename)) {
