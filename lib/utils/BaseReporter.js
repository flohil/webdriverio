...
class BaseReporter extends events.EventEmitter {
    constructor (config) {
        ...
        this.on('test:unverified', (test) => {
            ...
            this.setResults(test, 'unverified')
        })
    ...
    setResults (data, status) {
        if (this.startedSpecs) {
            ...
            if (data.arguments && data.arguments.date) {
                this.specResults[this.currentSpecId][this.currentCriteriaId].date = data.arguments.date.value
                this.specResults[this.currentSpecId][this.currentCriteriaId].manual = true
            } else {
                this.specResults[this.currentSpecId][this.currentCriteriaId].date = this.config.date
            }
        } else {
