...
class BaseReporter extends events.EventEmitter {
    ...
    results (counts, duration) {
        let fmt

        const total = 0 + counts.passes + counts.pending + counts.unverifieds + counts.failures + counts.brokens

        fmt = this.color('green', '%d passing') + this.color('light', ` (${(counts.passes / total * 100).toFixed(1)}%)`)
        console.log(fmt, counts.passes || 0)

        // pending
        if (counts.pending) {
            fmt = this.color('pending', '%d skipped') + this.color('light', ` (${(counts.pending / total * 100).toFixed(1)}%)`)
            console.log(fmt, counts.pending)
        }

        // unverifieds
        if (counts.unverifieds) {
            fmt = this.color('unverified', '%d unverified') + this.color('light', ` (${(counts.unverifieds / total * 100).toFixed(1)}%)`)
        ...
        if (counts.failures || counts.brokens) {
            if (counts.failures > 0) {
                fmt = this.color('fail', '%d failing') + this.color('light', ` (${(counts.failures / total * 100).toFixed(1)}%)`)
                console.log(fmt, counts.failures)
            }

            if (counts.brokens > 0) {
                fmt = this.color('broken', '%d broken') + this.color('light', ` (${(counts.brokens / total * 100).toFixed(1)}%)`)
    ...
    epilogue () {
        ...
        this.results(testcaseCounts, this.testcaseDuration)
        ...
        this.results(counts, this.stats.getSpecsDuration())
