        ...
        failures.forEach((test, i) => {
            ...
            if (test.errs && test.errs.length > 0) {
                test.errs.forEach(function (err, j) {
                    ...
                    const efmt = self.color(errMessageColor, '%s') + self.color('error stack', '\n%s\n')

                    self.log(efmt, err.message, err.stack)
                })
            } else if (test.unvalidated) {
                const efmt = this.color('unvalidated', '%s\n')
                ...
                if (test.errs && test.errs.length > 0) {
                    ...
                    this.log(efmt, test.err.message)
                }
            } else {
                ...
                this.log(efmt, this.trim(test.err.message), test.err.stack)
            }
        })
    }
